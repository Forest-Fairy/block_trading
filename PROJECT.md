...
## 项目架构
### 后端技术栈
 springboot 4, spring-cloud, spring-embabel, mybatis-flex, jimmer
### 前端技术栈
...
- 根目录 `pom.xml` 是当前仓库的构建入口，当前只聚合 `block_trading_docs`；`block_trading_bom`、`block_trading_server`、`block_trading_client` 和 `block_trading_deployment` 是保留名称和边界的目标模块，目录与实现尚未创建。
- 目标根模块固定为 `block_trading_docs`、`block_trading_bom`、`block_trading_server`、`block_trading_client`、`block_trading_deployment`。`block_trading_bom` 统一后端依赖与插件版本；`block_trading_server` 是 Maven 后端根；`block_trading_client` 是独立 Node 多端前端根，不进入 Maven Reactor；`block_trading_deployment` 只承载镜像、环境清单、数据库迁移、发布回滚和运维脚本。
