...
1. Maven 根父工程 artifactId 固定为 `block_trading`，目标根子模块固定为 `block_trading_docs`、`block_trading_bom`、`block_trading_server`、`block_trading_client` 和 `block_trading_deployment`；除独立 Node 工程 `block_trading_client` 外，其余模块在实际创建后进入 Maven Reactor。
2. 所有 Maven artifactId 使用小写字母、数字和下划线；目标后端模块沿用 `block_trading_*` 前缀。Java 包继续使用 `io.spray.qh`，不因 artifactId 使用下划线改变包命名。
...
本图是目标模块架构图，不表示当前所有模块都已创建。当前根 Maven 只聚合 `block_trading_docs`；后续将由根工程聚合 `block_trading_bom`、`block_trading_server` 与 `block_trading_deployment`，`block_trading_client` 作为同级独立 Node 工程存在且不进入 Maven Reactor。启动装配、部署交付和编译期依赖不增加第五个 Runtime 层。
...
block_trading/
  ...
  block_trading_client/                               非 Maven Node 工程根
...
- 当前根 `pom.xml` 只聚合 `block_trading_docs`；未来 `block_trading_bom`、`block_trading_server` 与 `block_trading_deployment` 直接由 `block_trading` 聚合，后端层模块只能挂在 `block_trading_server` 下。
- `block_trading_bom` 只提供 Spring、Spring Cloud、MyBatis-Flex、测试与构建插件的版本/依赖管理，不承载生产业务代码或启动逻辑。
- `block_trading_client` 是前端工程边界，不是 Maven 父模块；各终端应用使用自己的 `package.json`、构建工具和发布产物。
...
| `block_trading_a_social` | `block_trading_a_engagement_api/adapter` | 通知、客服、关键送达 | R1 | `block_trading_a_engagement_test` |
...
| `block_trading_d_social` | `block_trading_d_engagement_api/adapter` | 通知、投递、客服 | `block_trading_d_engagement_test` |
...
| `block_trading_i_repository_oracle` | 各领域仓储、ContextSnapshot、Outbox/Inbox、显式 SQL | `block_trading_i_repository_oracle_test` |
...
```text
block_trading_<layer>_<capability>/                         聚合父 POM，不被生产代码依赖
...
当前根工程可用 `mvn -q -pl block_trading_docs verify` 验证文档模块；`block_trading_system_test` 尚未创建，不能宣称当前已具备跨层测试。需要可执行服务时，先在对应业务模块内创建并测试 `*_boot`，再由部署资产消费其 JAR。
...
1. 全部 Maven artifactId 均为下划线命名；根坐标为 `block_trading`，目标根子模块为 `block_trading_docs`、`block_trading_bom`、`block_trading_server`、`block_trading_deployment`；`block_trading_client` 是同级非 Maven 工程，不建立统一 Runtime。
2. Reactor 图和目录能定位 `block_trading_server` 下的四层、层内分类、领域模块、测试父模块、系统测试根，`block_trading_client` 下的多端边界，以及 `block_trading_deployment` 下的部署号清单、共享脚本和按单元交付资产。
