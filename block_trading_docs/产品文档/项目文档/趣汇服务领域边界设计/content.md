...
| `region-policy` 区域与访问策略 | 支撑 | R1 | 区域树、业务开关、管理员角色/数据范围、策略版本、服务路由、区域应急路由 | 用户资料、帖子可见结论、订单/物流主状态、审批流程记录 |
...
- **聚合与写入权：** 区域、区域能力开关、管理员角色授予、角色权限、数据范围、业务策略版本、服务责任路由。
- **关键不变量：** 区域管理员只能在有效授权区域执行指定动作；高风险策略必须先获得 `governance` 审批，区域策略只在审批完成后产生可生效版本；区域关闭不应撤销已完成订单或用户的售后入口。
- **同步 API：** 区域准入、管理员数据范围、策略有效版本、区域服务责任查询。
