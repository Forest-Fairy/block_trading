...
import type { ActivityType, ViewerMode } from "@/prototype/data"

type DraftFields = Record<string, string>
export type CreateType =
  "拼单" | "拼车" | "线下组队" | "线上开黑" | "普通求助" | "紧急求助"
...
function guidanceFor(type: CreateType) {
  const copy: Record<CreateType, string> = {
    ...
    线下组队:
      "已选择线下组队。请一次说清活动、集合时间、地点、人数、费用和参与要求。",
    线上开黑:
      "已选择线上开黑。请一次说清游戏、平台、开局时间、人数和段位或语音要求。",
    普通求助:
      "已选择普通求助。请一次说清需要什么帮助、时间、地点、报酬和最晚响应时间。",
    紧急求助:
      "已选择紧急求助。如遇人身危险请优先拨打当地紧急电话；再描述需要的帮助、地点、时效和最晚响应时间。",
  }
  return copy[type]
}
...
export function AssistantChatPage({
  ...
  const [messages, setMessages] = useState<string[]>(
    action === "service"
      ...
      : initialDraft?.createType
        ? [
            "已恢复未完成草稿。你可以继续补充，或返回类型选择。",
            guidanceFor(initialDraft.createType),
          ]
  ...
  const [round, setRound] = useState(
    Object.keys(initialDraft?.form ?? {}).length ? 1 : 0
  )
  ...
  useEffect(() => {
    if (
      viewerMode !== "guest" &&
      !isService &&
      (createType || Object.keys(draft).length)
    ) {
      onSaveDraft({ createType, form: draft })
    }
  }, [createType, draft, isService, onSaveDraft, viewerMode])
      ...
      <div className="assistant-chat-page">
        <header className="assistant-chat-header">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-xs text-muted-foreground">趣汇助手 · 游客预览</p>
            <h1 className="truncate text-base font-bold">
              {actionTitle[action]}
            </h1>
          </div>
          <Sparkles className="mr-2 text-primary" size={21} />
        </header>
        <section className="assistant-chat-scroll">
          <Card className="border-primary/20 bg-secondary/60">
            <CardContent className="p-4">
              ...
              <p className="mt-2 text-xs leading-5 text-muted-foreground">
                完成身份信息认证后将自动成为
                VIP，可使用智能建帖、近邻求助和在线客服。游客访问不会创建草稿、帖子或会话。
              </p>
            </CardContent>
          </Card>
  ...
  const send = () => {
    ...
    setStreaming(true)
    const force = /改|改成|改为|换成/.test(message) ? ["primaryOccursAt"] : []
    ...
    const firstRoundPatches: Record<ActivityType, Array<[string, string]>> = {
      拼单: [
        ...
        [
          "primaryOccursAt",
          force.length ? "2026-08-15T19:00" : "2026-08-16T18:00",
        ],
        ["productName", "山姆零食组合包"],
      ],
      拼车: [
        ...
        [
          "primaryOccursAt",
          force.length ? "2026-08-14T19:00" : "2026-08-14T19:30",
        ],
        ["originName", "萧山机场"],
      ...
      线下组队: [
        ...
        [
          "primaryOccursAt",
          force.length ? "2026-08-15T09:00" : "2026-08-16T07:30",
        ],
      ...
      线上开黑: [
        ...
        [
          "primaryOccursAt",
          force.length ? "2026-08-15T19:00" : "2026-08-16T20:00",
        ],
        ["gameCategory", "MOBA"],
        ["gameName", "王者荣耀"],
      ],
      近邻互助: [
        ...
        [
          "primaryOccursAt",
          force.length ? "2026-08-15T14:00" : "2026-08-14T14:00",
        ],
        ["helpCategory", "借用"],
  ...
  const moveRecording = (event: PointerEvent<HTMLButtonElement>) => {
    if (
      ...
      event.clientY < recordStart.current - 52
    ) {
  ...
  return (
    <div className="assistant-chat-page">
      <header className="assistant-chat-header">
        ...
        {isService ? (
          <Headphones className="mr-2 text-primary" size={21} />
        ) : null}
      </header>
          ...
          <div className="assistant-image-picker">
            {images.length ? (
              images.map((name, index) => (
                <div className="assistant-image-item" key={name}>
                  <span>{name}</span>
                  {index === 0 ? <b>封面</b> : null}
                </div>
              ))
              ...
              <div
                className="assistant-poster"
                style={{ background: posters[posterIndex] }}
              >
                <span>{title}</span>
              </div>
            )}
            <label className="assistant-add-image" htmlFor="assistant-images">
              <ImagePlus size={20} />
              添加图片
            </label>
            <input
            ...
            {requiredFields.map((field) => (
              <label key={field.key} className="block space-y-1.5">
                <span className="text-xs font-semibold">
                  {field.label}{" "}
                  <span className="text-[var(--qh-coral)]">*</span>
                </span>
                <Input
                  ...
                  onChange={(event) =>
                    updateManualField(field.key, event.target.value)
                  }
                />
              </label>
            ))}
          ...
          {advancedFields.length ? (
            <>
              <Button
                ...
                onClick={() => setManualAdvancedOpen((open) => !open)}
              >
        ...
        <section className="assistant-chat-scroll" aria-live="polite">
          {isService ? (
            ...
            <div className="assistant-type-options">
              {options[action as Exclude<AssistantAction, "service">].map(
                (type) => (
                  <Button
                    key={type}
                    type="button"
              ...
              <CardContent className="p-3">
                <div className="mb-2 flex items-center justify-between gap-2">
                  <span className="flex items-center gap-1 text-xs font-bold">
                    ...
                    {streaming ? "正在回填草稿" : "建帖草稿"}
                  </span>
                  <span className="text-[0.6875rem] text-muted-foreground">{`{ form, force }`}</span>
                </div>
                {Object.entries(draft).map(([field, value]) => (
                  <div className="assistant-draft-field" key={field}>
                    <span>{fieldLabel(field)}</span>
                    <strong>{value}</strong>
                  </div>
                ))}
                {Object.entries(candidates).map(([field, value]) => (
                  <div className="assistant-candidate" key={field}>
                    <span>
                      {fieldLabel(field)} 建议改为“{value}”
                    </span>
                    <Button
                      type="button"
                      size="xs"
                      onClick={() => adoptCandidate(field)}
                    >
                      采用
                    </Button>
                  </div>
                ...
                {missing.length ? (
                  <p className="mt-2 text-xs text-[var(--qh-coral)]">
                    待补充：
                    {missing
                      .slice(0, 2)
      ...
      {!manual ? (
        <section className="assistant-input-bar">
          ...
          <Button
            ...
            aria-label="发送"
            onClick={send}
            disabled={!input.trim() || (!isService && !createType) || streaming}
          >
            <Send />
          </Button>
        </section>
      ...
      {recording ? (
        <div className="assistant-recording-state">
          <Mic size={18} />
          {recordCancel ? "松开取消录音" : "正在录音，上划取消，松开转文字"}
        </div>
      ) : null}
      <Dialog open={saveConfirmOpen} onOpenChange={setSaveConfirmOpen}>
        <DialogContent showCloseButton={false}>
          ...
          <DialogFooter>
            ...
            <Button type="button" onClick={saveDraftAndLeave}>
              保存草稿
            </Button>
          </DialogFooter>
        </DialogContent>
...
function ServiceHint() {
  return (
    <Card className="border-[var(--qh-blue)]/20 bg-[var(--qh-blue-soft)]">
      <CardContent className="flex gap-2 p-3 text-xs text-[var(--qh-blue)]">
        <Headphones size={17} className="shrink-0" />
        客服回复以会话消息和站内通知为准，请勿在聊天中发送身份证、支付密码等敏感信息。
      </CardContent>
    </Card>
  )
}
